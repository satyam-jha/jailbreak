...
## Heat now affects the scene: alarms pulse at high alert and guards appear at
## critical alert. All art remains procedural.
...
var _caption: Label
var _actions := ["WATCHING THE DOOR", "CHECKING THE WALL", "KEEPING LOOKOUT", "PRETENDING TO WORK", "ARGUING QUIETLY"]
var _success_actions := ["NICE. THAT WORKED.", "ACTUALLY A GOOD IDEA.", "THE PLAN IS WORKING.", "NOBODY SAW THAT.", "KEEP MOVING."]
var _failure_actions := ["THAT WENT BADLY.", "WHO HAD THAT IDEA?", "WE SHOULD NOT HAVE DONE THAT.", "EVERYONE ACT NORMAL.", "THIS IS FINE. IT IS NOT FINE."]
...
func set_mood(value: String) -> void:
	...
	for i in range(_actors.size()): _actors[i].expression = _expression_for(i)
	queue_redraw()
...
func _layout_actors() -> void:
	if _actors.is_empty() or size.x < 20.0: return
...
func _process(delta: float) -> void:
	...
	for i in range(_actors.size()):
		if i >= _base_positions.size(): continue
		var bob := sin(_time * (1.6 + i * 0.13) + i) * 2.2
		var sway := sin(_time * 0.8 + i * 1.7) * 1.4
		var target := _base_positions[i] + Vector2(sway, bob)
		if mood == "success": target.y -= absf(sin(_time * 4.0 + i)) * 5.0
		elif mood == "failure": target.x += sin(_time * 5.0 + i) * 2.5
	...
	if fmod(_time, 0.2) < delta: queue_redraw()

func _draw() -> void:
	draw_rect(Rect2(0, 0, size.x, HEIGHT), Palette.with_alpha(Palette.BG_DEEP, 0.10), false, 2.0)
	var heat := int(Game.heat)
	var alarm_alpha := 0.0
	if heat >= 60: alarm_alpha = 0.10 + 0.10 * absf(sin(_time * 5.0))
	elif heat >= 40: alarm_alpha = 0.05
	if alarm_alpha > 0.0:
		draw_rect(Rect2(0, 0, size.x, HEIGHT), Palette.with_alpha(Palette.DANGER, alarm_alpha), true)
		_draw_alarm_lights()
		if heat >= 75: _draw_guard_hint()
	var light_alpha := 0.10
	if mood == "success": light_alpha = 0.16
	elif mood == "failure": light_alpha = 0.20 + 0.08 * absf(sin(_time * 6.0))
...
func _draw_alarm_lights() -> void:
	...
	for x in [18.0, size.x - 18.0]:
		draw_circle(Vector2(x, 18), 7.0, Palette.with_alpha(Palette.DANGER, pulse))
		draw_circle(Vector2(x, 18), 13.0, Palette.with_alpha(Palette.DANGER, 0.12 * pulse))

func _draw_guard_hint() -> void:
	var y := 63.0
	for x in [26.0, size.x - 26.0]:
		draw_circle(Vector2(x, y), 11.0, Palette.with_alpha(Palette.INK, 0.72))
		draw_rect(Rect2(x - 12, y + 10, 24, 48), Palette.with_alpha(Palette.INK, 0.72), true)
...
func _action_for(index: int) -> String:
	...
	if mood == "failure": return _failure_actions[index % _failure_actions.size()]
	return _actions[index % _actions.size()]

func _expression_for(index: int) -> String:
	if mood == "success": return "happy" if index % 3 != 2 else "idle"
	if mood == "failure": return "scared" if index % 2 == 0 else "worried"
	return "idle"
