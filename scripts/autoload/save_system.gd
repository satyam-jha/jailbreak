extends Node
## Autoload: SaveSystem
## Settings plus one optional in-progress run.
...
func load_settings() -> void:
	...
	for key in data.keys():
		if settings.has(key): settings[key] = data[key]
...
func save_run() -> bool:
	...
	data["strategy_state"] = Strategy.to_dict()
	return _write(RUN_PATH, data)
...
func load_run() -> bool:
	...
	if int(data.get("version", 0)) != SAVE_VERSION:
		...
		return false
	var loaded := Game.from_dict(data)
	if loaded: Strategy.from_dict(data.get("strategy_state", {}))
	return loaded

func delete_run() -> void:
	if has_run(): DirAccess.remove_absolute(ProjectSettings.globalize_path(RUN_PATH))
