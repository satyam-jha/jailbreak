...
func _choice_card(choice: Dictionary) -> Control:
	...
	if bool(choice.get("auto", false)):
		...
		if available:
			var chance := _estimate(choice)
			head.add_child(UIKit.chip("%d%%" % chance, _chance_color(chance)))
	v.add_child(head)
	var notes := UIKit.hbox(8)
	if choice.has("hint"):
		notes.add_child(UIKit.label(str(choice["hint"]), 13, Palette.TEXT_FAINT))
	if int(choice.get("cost_money", 0)) > 0:
		notes.add_child(UIKit.chip("Costs $%d" % int(choice.get("cost_money", 0)), Palette.ACCENT))
...
func _render_actor_picker() -> void:
	...
	for actor in actors:
		...
		wrapper.add_child(UIKit.chip("%d%% likely" % estimate, _chance_color(estimate)))
...
func _resolve(actor: Prisoner) -> void:
	...
	Strategy.observe_outcome(_report.get("outcome", {}), _report.get("result"), actor)
	_state = State.RESULT
	_render()
...
func _render_result() -> void:
	...
	if check != null:
		...
		if result_check.tier == CheckResult.Tier.CRIT_SUCCESS or result_check.tier == CheckResult.Tier.SUCCESS:
			reaction_mood = "success"
		else:
			reaction_mood = "failure"
	reaction.set_mood(reaction_mood)
	_body.add_child(reaction)
	if check != null:
		_body.add_child(CheckPanel.new(check as CheckResult))
	...
	var cv := UIKit.vbox(4)
	consequence_box.add_child(cv)
	cv.add_child(UIKit.label("WHAT THIS CHANGED", 13, Palette.ACCENT))
	var warning := Strategy.warning_text()
	if not warning.is_empty():
		cv.add_child(UIKit.label(warning, 13, Palette.DANGER))
	var suspicion := int(strategy_after.get("suspicion", 0))
	var noise := int(strategy_after.get("noise", 0))
	var intel := int(strategy_after.get("intel", 0))
	var patrols := int(strategy_after.get("guard_attention", 0))
	var favors := int(strategy_after.get("favors", 0))
	var state_text := "Suspicion %d  •  Noise %d  •  Intel %d  •  Patrols %d  •  Favors %d" % [suspicion, noise, intel, patrols, favors]
	cv.add_child(UIKit.label(state_text, 13, Palette.TEXT_FAINT))
...
func _walk_away() -> void:
	...
	Strategy.observe_outcome({"heat": 5, "note": "The crew abandoned the room."}, null, null)
