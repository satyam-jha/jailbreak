...
func _numbers_panel(data: Dictionary) -> Control:
	var box := UIKit.panel(Palette.PANEL_DARK, Palette.BORDER, 14)
	...
	if not str(data["route"]).is_empty():
		v.add_child(UIKit.stat_row("Escape route", str(Content.get_escape_route(str(data["route"])).get("name", "-"))))

	v.add_child(UIKit.spacer(10))
	v.add_child(UIKit.section("The crew", Palette.TEAL))
	for member in Game.party:
		...
		info.add_child(UIKit.label(p.display_name, 15, Palette.TEXT))
		info.add_child(UIKit.label("%d/%d HP - %s - %d of %d checks passed" % [
			p.health, p.max_health, p.status_name(), p.checks_passed, p.checks_attempted], 12, Palette.TEXT_FAINT))
		row.add_child(info)
		v.add_child(row)

	v.add_child(UIKit.spacer(10))
	var score := UIKit.hbox(10)
	score.add_child(UIKit.label("FINAL SCORE", 16, Palette.TEXT_DIM))
	score.add_child(UIKit.spacer())
	score.add_child(UIKit.label(str(data["score"]), 34, Palette.ACCENT))
	v.add_child(score)
	v.add_child(UIKit.label("RUN SEED: %d" % int(data["seed"]), 13, Palette.TEXT_FAINT))
	return box
...
func _story_panel(data: Dictionary) -> Control:
	...
	if notable.is_empty():
		v.add_child(UIKit.paragraph("Nothing worth writing down happened, which is its own kind of achievement.", 15, Palette.TEXT_FAINT))
	else:
		# Newest last reads like a story; oldest first reads like a report.
		for line in notable:
			var row := UIKit.hbox(8)
			row.add_child(UIKit.label("-", 15, Palette.PURPLE))
			row.add_child(UIKit.paragraph(str(line), 15, Palette.TEXT))
			v.add_child(row)

	v.add_child(UIKit.spacer(8))
	v.add_child(UIKit.section("Power-ups carried", Palette.ACCENT))
	if Game.powerups.is_empty():
		v.add_child(UIKit.paragraph("None. A purist run, or a short one.", 14, Palette.TEXT_FAINT))
	for offer in Game.powerups:
		var def: Dictionary = offer.get("def", {})
		var row2 := UIKit.vbox(2)
		var head := UIKit.hbox(8)
		head.add_child(UIKit.chip(str(def.get("name", "?")), Palette.ACCENT))
		if bool(offer.get("has_catch", false)):
			head.add_child(UIKit.chip("HAD A CATCH", Palette.DANGER))
		head.add_child(UIKit.spacer())
		row2.add_child(head)
		# The trap is always spelled out at the end, even if it never fired.
		if bool(offer.get("has_catch", false)):
			row2.add_child(UIKit.paragraph(str(PowerupSystem.catch_for(offer).get("reveal_text", "")), 13,
				Palette.DANGER if bool(offer.get("revealed", false)) else Palette.TEXT_FAINT))
			if not bool(offer.get("revealed", false)):
				row2.add_child(UIKit.paragraph("...and it never went off. You got away with it.", 13, Palette.SUCCESS))
		v.add_child(row2)
	return box
